...
import { SiderItem } from '@/types/component';
import {
    AppstoreAddOutlined,
    CreditCardOutlined,
    InfoCircleOutlined,
...
const Sider: React.FC = () => {
    ...
    return (
        <>
            <div className={`sider-wrapper ${!expanded ? "sider-closed" : ""}`} >
                <div className="header-title">
                    <img src={logo} alt="logo" className="logo" onClick={() => setExpanded(true)} />
                    {expanded &&
                        <button className="logoBtn" onClick={() => setExpanded(!expanded)} >
                ...
                {items.map((item) => {
                    ...
                    {/** @MODERATE , @Dependency ROUTING-PLAN  */}
                    {/** Fix this active bug after completion of proper routings */}
                    const isActive = location.pathname.startsWith(item.key);
                    
                    return (
                        <div key={item.key} className={`sider-item ${isActive ? "active" : ""}`} onClick={item.onClick}>
                            <div className={`icon-slot  ${!expanded ? "icon-shrinked" : ""}`}>
                                {item.icon}
                            </div>
                            <div className={`label-slot  ${!expanded ? "label-hidden" : ""}`}>
                                {item.label}
                            </div>
                        </div>
                    )
